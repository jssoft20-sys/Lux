...
DB_TRANSIENT_ERRORS = (OperationalError, InterfaceError, DisconnectionError, PoolTimeoutError)  # retried once, never shown as a failure at first sight
...
class Ctx:
    ...
    # --------------------------------------------------------- screens
    def panel(self, text: str, markup: dict | None = None, *, photo: bytes | Path | None = None, state: str | None = None, data: dict[str, Any] | None = None, protect: bool = True, keep_previous: bool = False, caption: bool = False) -> int:
        ...
        if photo is None and message_id and current_kind == "text":
            try:
                ...
                if exc.fatal_for_chat:
                    raise
        if photo is None and message_id and current_kind == "photo" and caption:
            try:
                self.bot.safe_edit_caption(self.chat_id, message_id, text, markup)
...
class MainBot:
    ...
    def _safe(self, fn, payload: dict[str, Any]) -> None:
        ...
        retry a second later; anything that still fails is reported with a separate message —
        the client's screen, its buttons and the step they were on stay exactly as they were,
        so «попробуйте ещё раз» really means try the same step again."""
        for attempt in (1, 2):
            try:
                fn(payload)
                return
                ...
                return
            except DB_TRANSIENT_ERRORS as exc:
                if attempt == 1:
                    logger.warning("database hiccup in handler — retrying once: %s", str(exc)[:200])
                    time.sleep(1.0)
                    continue
                logger.exception("handler failed after a database retry")
            except Exception:
    ...
    def ask_id(self, ctx: Ctx, data: dict[str, Any], saved: list[tuple[str, str]] | None = None, error: str = "") -> None:
        ...
        # an error on a photo screen is shown in the caption of that same screen (nothing new is sent)
        same_photo = bool(error) and str(ctx.data.get("panel_kind")) == "photo"
        ctx.panel(text, inline_keyboard(*rows), photo=None if same_photo else photo, caption=same_photo, state="choose_id" if saved else "wait_id", data=data)
    ...
    def on_id(self, ctx: Ctx, text: str) -> None:
        ...
        self.progress(ctx, ctx.T("checking_id"), self.cancel_kb(ctx))
        try:
    ...
    def ask_code(self, ctx: Ctx, data: dict[str, Any], error: str = "") -> None:
        ...
        same_photo = bool(error) and str(ctx.data.get("panel_kind")) == "photo"
        ctx.panel(text, markup, photo=None if same_photo else photo, caption=same_photo, state="wait_code", data=data)
    ...
    def on_code(self, ctx: Ctx, text: str) -> None:
        ...
        if len(code) < 3:
            ...
            return
        self.progress(ctx, ctx.T("checking_code"))
        nonce = str(ctx.data.get("nonce") or secrets.token_hex(6))
    ...
    def warm_up(self) -> None:
        ...
        try:
            with transaction() as db:
                ...
                bot_texts.menu_labels(db)
                cash_service.list_cashes(db, enabled_only=True)
            self.premium()
        except Exception as exc:
