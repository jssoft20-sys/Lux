"""PayGo client bot (@PayGoXBot): inline-only UX, one editable panel per chat.
...
from paygo.models import BotSession, Deposit, Notification, PaymentCash, User, Withdrawal
from paygo.services import bot_state, elqr, settings_store
...
class MainBot:
    ...
    def show_referrals(self, ctx: Ctx, note: str = "") -> None:
        ...
        if ctx.lang == "ru":
            ...
            share = f"https://t.me/share/url?url={link}&text=Пополняй и выводи через PayGo без комиссии"
        else:
            ...
            share = f"https://t.me/share/url?url={link}&text=PayGo аркылуу комиссиясыз толуктаңыз"
        if note:
            text += "\n\n" + note
