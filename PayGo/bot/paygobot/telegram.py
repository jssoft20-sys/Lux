...
CONNECT_TIMEOUT = 6.0  # a dead network must fail fast, not hold a handler for the whole read timeout
...
class TelegramClient:
    ...
    def rebuild_http(self) -> None:
        """Drop every pooled connection and start over (self-heal after a streak of network failures:
        stale keep-alive sockets, a changed DNS answer, a proxy that silently died)."""
        old, self._http = self._http, self._new_http()
        self.rebuilds += 1
        try:
            old.close()
        except Exception:  # pragma: no cover - closing a broken pool must never hurt
            pass
        logger.warning("telegram http client rebuilt (%d)", self.rebuilds)
        ...
        while True:
            ...
            try:
                if files:
                    ...
                    response = self._http.post(url, data=data, files=files, timeout=httpx.Timeout(timeout or 60, connect=CONNECT_TIMEOUT))
                else:
                    response = self._http.post(url, json=payload or {}, timeout=httpx.Timeout(timeout or 20, connect=CONNECT_TIMEOUT))
                body = response.json()
    ...
    def send_chat_action(self, chat_id: int, action: str = "typing") -> bool:
        """«печатает…» in the client's chat — the bot is visibly alive the instant a message arrives,
        even while a cash-desk API or the database is still answering. Never raises."""
        try:
            self.call("sendChatAction", {"chat_id": int(chat_id), "action": action}, timeout=6, retries=0)
            return True
        except TelegramError as exc:
