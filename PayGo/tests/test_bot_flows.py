...
def test_photo_step_shows_progress_and_errors_in_place(bot, fake_provider):
    """A step with a photo (ID screenshot) keeps one message: errors edit its caption,
    the check shows «Проверяем ID…» on its buttons — nothing new is sent, nothing is lost."""
    from pathlib import Path

    from paygo.services.cashes import get_cash

    folder = Path(bot.settings.data_dir) / "uploads" / "cash"
    folder.mkdir(parents=True, exist_ok=True)
    (folder / "1xbet-deposit.jpg").write_bytes(b"\xff\xd8fake")
    with transaction() as db:
        get_cash(db, "1xbet").deposit_photo = "uploads/cash/1xbet-deposit.jpg"
    text(bot, "/start")
    text(bot, "Пополнить")
    pick_cash(bot)
    assert bot.client.last[0] == "photo" and state_of()[1]["panel_kind"] == "photo"
    sent_before = len([c for c in bot.client.calls if c[0] in {"send", "photo"}])
    text(bot, "abc")  # not an ID → the error lands in the caption of the same photo
    kind, body, markup = bot.client.last
    assert kind == "edit_caption" and "только цифры" in body and state_of()[1]["panel_kind"] == "photo"
    assert len([c for c in bot.client.calls if c[0] in {"send", "photo"}]) == sent_before
    text(bot, "123456")
    progress = [m for k, _, m in bot.client.calls if k == "markup" and m]
    labels = [b["text"] for m in progress for row in m["inline_keyboard"] for b in row]
    assert any("Проверяем ID" in label for label in labels)
    assert state_of()[0] == "wait_amount" and "Введите сумму" in bot.client.last[1]


def test_warm_up_touches_settings_and_cashes(bot):
    bot.warm_up()  # must never raise: it only pre-loads caches
    assert bot.premium()["enabled"] is True


def test_database_hiccup_is_retried_and_never_wipes_the_screen(bot, monkeypatch):
    """A dropped connection on the first attempt is retried a second later; the client's step,
    screen and buttons stay in place — only a persistent failure gets a separate error message."""
    from paygobot import main_bot
    from sqlalchemy.exc import OperationalError

    monkeypatch.setattr(main_bot.time, "sleep", lambda s: None)
    text(bot, "/start")
    text(bot, "Пополнить")
    _, _, panel_before = state_of()
    calls = {"n": 0}
    original = bot.on_message
    ...
    monkeypatch.setattr(bot, "on_message", flaky)
    sent_before = len(bot.client.calls)
    text(bot, "Вывести")  # first attempt hits the dead connection, the retry succeeds
    assert calls["n"] == 2
    assert "Что-то пошло не так" not in " ".join(str(c[1]) for c in bot.client.calls[sent_before:])
    assert state_of()[0] == "choose_cash" and "Выберите сайт для вывода" in bot.client.last[1]
    # a persistent failure: the error comes as a separate message, the current screen is untouched
    calls["n"] = -5
    panel_kind, panel_id = state_of()[1].get("panel_kind"), state_of()[2]
    monkeypatch.setattr(bot, "on_message", lambda message: (_ for _ in ()).throw(OperationalError("x", {}, Exception("down"))))
    text(bot, "Пополнить")
    kind, body, _ = bot.client.last
    assert kind == "send" and "Что-то пошло не так" in body
    assert state_of()[0] == "choose_cash" and state_of()[2] == panel_id and state_of()[1].get("panel_kind") == panel_kind
