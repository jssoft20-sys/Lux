...
def database(tmp_path, monkeypatch):
    ...
    from paygo import db as dbmod
    from paygo import models  # noqa: F401
...
def seeded(database):
    ...
    from paygo.models import PaymentRequisite
    from paygo.seed import seed_defaults
    from paygo.services.cashes import get_cash, update_cash
...
def client(seeded):
    ...
    from paygo.app import create_app
...
def logged(client, admin):
    r = client.post("/paygo/api/auth/login", json={"username": admin["username"], "password": admin["password"]})
