...
`paygobot/dispatcher.py`: мгновенный `answerCallbackQuery` и статус «печатает…» на каждое сообщение/нажатие (и повторно каждые 3 с, пока обработчик ждёт кассу или Claude), дедупликация update_id/callback_id, FIFO по чату, отбрасывание повторных нажатий (1 c) и нажатий во время незавершённого перехода, персистентный offset (сбрасывается на диск и перед аварийным перезапуском по сторожевому потоку). Серия неудачных `getUpdates` сначала пересоздаёт HTTP-клиент (10 подряд), затем завершает процесс (60 подряд) — systemd поднимает свежий. После простоя старые текстовые сообщения из очереди (старше 15 минут) не проигрываются как новые команды; фото, файлы и нажатия сохраняются. Экран бота — одно сообщение-панель, которое редактируется на месте (`bot_sessions`); ошибка на шаге с фото меняет подпись того же фото, проверка ID/кода показывается прямо на кнопках экрана. Фото QR пополнения отправляется отдельным сообщением и заменяется итоговым текстом после оплаты/истечения.

`paygobot/runtime.py`: сторожевой поток (перезапуск при остановке опроса), keep-alive соединения с БД раз в 30 с (первый клиент после тихого дня не ждёт переподключения), прогрев настроек и касс при старте. `paygo/db.py`: TCP keepalive и `connect_timeout` для PostgreSQL, `pool_pre_ping`, LIFO-пул — мёртвое соединение обнаруживается за секунды, а не на первом обработчике.

## Тексты и сценарий клиентского бота

Все клиентские тексты — шаблоны в `system_settings` (`services/bot_texts.py`: подстановки `{name}`, `{amount}`…, HTML, premium-эмодзи `[emoji:ID:😎]`), редактируются в панели (Настройки → Тексты). Сценарий: reply-клавиатура Пополнить / Вывести / Помощь → выбор сайта (inline, эмодзи кассы) → ID (фото-подсказка кассы) → сумма → фото-карточка QR (`services/qr.py: render_pay_card`, водяной знак, надпись поверх кода, «ОТСКАНИРУЙТЕ QR / В любом банке») + кнопки банков → просьба прислать чек. Вывод: QR кошелька → ID (фото) → код (фото) → заявка. Истечение/отмена: карточка удаляется, клиент получает «Пополнение отменено…». Успех: «✅ Пополнено».
