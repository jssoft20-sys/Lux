...
(function () {
  ...
  function txMini(tx, s) {
    /* compact request row: tap → the request card (modal), «Чек» → the receipt image */
    ...
    return h('button', { class: 'tx-mini ' + (dep ? 'deposit' : 'withdraw'), type: 'button', onclick: () => { if (s) s.close(); openTxSheet(tx.kind, tx.id); } },
      ...
      h('span', { class: 'tx-mini-side' }, txStatus(tx), receipt ? h('span', { class: 'tx-mini-btns' }, receipt) : null));
  }
  ...
  async function openTxSheet(kind, id, opts) {
    /* the one and only request view: a bottom card with the amount, the client, the details, the history
       and every action (Зачислить / Перевёл, Изменить, Отменить, Чек, Написать, Отказать). Drag up for full screen. */
    opts = opts || {};
    const path = kind === 'deposit' ? 'deposits' : 'withdrawals';
    const s = sheet({ title: (kind === 'deposit' ? 'Пополнение' : 'Вывод') + ' # ' + id, body: loader(2), onClose: opts.onClose });
    let stamp = '';
    ...
    load(true);
    watchChanges(s.el, () => load(false)); /* a payment or a status change made elsewhere refreshes the open card */
    return s;
  }
  function txDeepLink(shell, kind, id) {
    /* #/deposits/12 or #/withdrawals/7 (a notification, a chat, an old bookmark): the home list with the
       request card on top — there is no separate request page any more */
    homeView(shell);
    if (!id) return;
    const hash = '#/' + (kind === 'deposit' ? 'deposits' : 'withdrawals') + '/' + id;
    openTxSheet(kind, Number(id), { onClose: () => { if (location.hash === hash) { history.replaceState(null, '', '#/home'); state.route = parseHash(); } } });
  }
