  ...
  async function gatewayView(shell) {
    ...
    const draw = async () => {
      try {
        ...
        bl.items.forEach((l) => box.appendChild(h('div', { class: 'card wallet-card' }, h('span', { class: 'ico' }, l.emoji ? h('span', { style: { fontSize: '20px' } }, l.emoji) : svg('bank', 20)), h('div', { style: { minWidth: 0 } }, h('b', null, l.name), h('small', null, l.prefix || 'показ QR-картинки'), l.kind !== 'qr' ? h('div', { class: 'tag-row' }, editable(l.emoji, { render: (v) => 'эмодзи: ' + (v || '—'), save: (v) => api('/bank-links', { method: 'POST', body: { key: l.key, emoji: v } }) }), editable(l.custom_emoji_id, { render: (v) => 'premium ID: ' + (v || '—'), save: (v) => api('/bank-links', { method: 'POST', body: { key: l.key, custom_emoji_id: v.trim() } }) })) : null), switchEl(l.enabled, async (v) => { await api('/bank-links', { method: 'POST', body: { key: l.key, enabled: v } }); }))));
  ...
  async function settingsView(shell, forcedTab) {
    ...
    try {
      ...
      const drawTab = (key) => {
        ...
        if (tab[0] === 'bot') bottom = h('div', { class: 'card section-card' }, h('h2', null, 'Premium-эмодзи'), h('div', { class: 'small muted', style: { marginBottom: '8px' } }, 'Telegram разрешает premium-эмодзи только ботам с коллекционным username с fragment.com. Кнопка отправит тестовое сообщение через клиентского бота и покажет ответ Telegram.'), h('button', { class: 'outline-btn blue', onclick: async (e) => { const chat = await promptDialog('Проверка premium-эмодзи', 'Ваш Telegram ID (сначала напишите боту /start). Пусто — первый ID из ADMIN_TELEGRAM_CHAT_IDS', '700100200'); if (chat === null) return; const b = e.currentTarget; b.disabled = true; try { const rr = await api('/settings/premium-test', { method: 'POST', body: { chat_id: chat.trim() || null } }); if (rr.sent) toast('Отправлено — проверьте чат с ботом: эмодзи должно быть анимированным', 'ok', 6000); else toast('Telegram отказал: ' + rr.description + (rr.hint ? ' — ' + rr.hint : ''), 'err', 12000); } catch (ex) { err(ex); } b.disabled = false; } }, svg('bolt', 14), 'Проверить premium-эмодзи'));
        if (tab[0] === 'texts') top = h('div', { class: 'card section-card' }, h('h2', null, 'Подстановки'), h('div', { class: 'small muted', style: { marginBottom: '6px' } }, 'Можно использовать HTML: <b>, <i>, <blockquote>. Premium-эмодзи: [emoji:ID:😎]'), h('div', { class: 'placeholder-list' }, ['{name}', '{support}', '{brand}', '{cash}', '{emoji}', '{player}', '{amount}', '{cur}', '{min}', '{max}', '{minutes}', '{left}', '{reason}', '{sla}', '{city}', '{address}', '{have}', '{need}'].map((x) => h('code', null, x))), h('div', { class: 'btn-row' }, h('button', { class: 'outline-btn', onclick: async () => { if (await confirmDialog('Вернуть все тексты бота к стандартным (с premium-эмодзи как в образце)? Ваши правки текстов будут удалены.', 'Сбросить', true)) { try { const rr = await api('/settings/reset', { method: 'POST', body: { keys: ['texts'] } }); Object.assign(values, rr.values); toast('Тексты сброшены: ' + rr.reset.length, 'ok'); drawTab('texts'); } catch (ex) { err(ex); } } } }, svg('refresh', 14), 'Сбросить тексты к стандартным')));
