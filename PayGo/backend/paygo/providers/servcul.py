...
class ServculAdapter(BaseAdapter):
    ...
    def lookup_player(self, player_id: str) -> ProviderResult:
        ...
        # read-only: a shorter timeout and one retry keep the «Проверяем ID…» step quick and reliable
        status, data = http_json("GET", f"{self.base_url}/Users/{pid}?{query}", headers=self._headers(sign), label="servcul.lookup", timeout=LOOKUP_TIMEOUT, retries=1)
        root = mapping(data)
