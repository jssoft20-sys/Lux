...
logger = logging.getLogger("paygo.providers")
...
_CLIENT = httpx.Client(
    ...
    headers={"User-Agent": "PayGo/1.0", "Accept": "application/json"},
    follow_redirects=False,
)
# Read-only calls (ID lookup) answer the client while they wait: a shorter read timeout and one
# retry keep the bot snappy when the cash desk is sluggish. Financial calls keep the long timeout.
LOOKUP_TIMEOUT = httpx.Timeout(connect=4.0, read=12.0, write=8.0, pool=4.0)
...
def http_json(
    ...
    *,
    ...
    """Single HTTP call. Financial calls are never retried (``retries=0``, the default): a request
    that may have reached the provider must not be duplicated. Read-only lookups may pass
    ``retries=1`` — a network hiccup then costs a second attempt instead of an error screen."""
    attempt = 0
    while True:
        attempt += 1
        started = time.monotonic()
        try:
            kwargs: dict[str, Any] = {"json": json, "headers": headers or {}}
            if timeout is not None:
                kwargs["timeout"] = timeout
            response = (client or _CLIENT).request(method, url, **kwargs)
            ...
            logger.warning("[%s] %s %s failed: %s%s", label, method, url.split("?")[0], type(exc).__name__, " — retrying" if attempt <= retries else "")
            if attempt <= retries:
                time.sleep(0.5)
                continue
