...
def _make_engine(url: str) -> Engine:
    ...
    if url.startswith("postgresql"):
        # A connection the server (or a NAT/firewall) silently dropped while the bot was quiet
        # is detected in seconds by TCP keepalives instead of hanging the first handler of the
        # day; a database that is down fails the connect fast instead of blocking for minutes.
        connect_args = {"connect_timeout": 5, "keepalives": 1, "keepalives_idle": 30, "keepalives_interval": 10, "keepalives_count": 3}
    return create_engine(
        ...
        pool_pre_ping=True,
        pool_recycle=900,
        pool_use_lifo=True,  # keep reusing the same warm connections; the rest idle out and are recycled
