...
logger = logging.getLogger("paygo")

CSP = (
    "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; "
    "media-src 'self' blob:; font-src 'self' data:; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; "
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(title="PayGo", version=__version__, docs_url=None, redoc_url=None, openapi_url=None)
    app.add_middleware(SecurityHeadersMiddleware)

    prefixed = FastAPI(title="PayGo API", docs_url=None, redoc_url=None, openapi_url=None)
    ...
    async def healthz():
        ...
        ok = ping()
        return JSONResponse({"ok": ok, "service": "paygo", "version": __version__}, status_code=200 if ok else 503)
    ...
    if base:
        ...
        async def root():
            return JSONResponse({"ok": True, "service": "paygo", "admin": base + "/"})
    else:
        app.mount("/", prefixed)
    return app
