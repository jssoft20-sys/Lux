...
def _client() -> Any:
    ...
    with _CLIENT_LOCK:
        if _CLIENT is None:
            # the client sees «печатает…» meanwhile, but a support answer must still arrive quickly:
            # a slow API round falls back to the rule-based answers instead of a long silence
            _CLIENT = anthropic.Anthropic(api_key=get_settings().anthropic_api_key, timeout=20.0, max_retries=1)
        return _CLIENT
