...
var PAGES=['home','chat','chats','dms','dm','tv','history','profile','bk','pay','support','u','devices','notifs','link','bots','news','bot','calls','contacts'];
/* 11.12: стартовая страница — Чаты, как в Telegram. Касса — центральная кнопка. */
function parseHash(){var hs=(location.hash||'').replace(/^#\/?/,'').split('/');var pg=hs[0]||'chats';return {page:pg,arg:hs[1]||''};}
...
function Nav(p){
 ...
 var map={chat:'chats',dm:'chats',dms:'chats',bot:'chats',bots:'chats',news:'chats',support:'chats',contacts:'chats'};
...
function App(){
 ...
 var notifAfter=useRef(0),toastSeq=useRef(0),newVer=useRef(false);var page=route.page,arg=route.arg;var [photo,setPhoto]=useState('');var [linkUrl,setLinkUrl]=useState('');var [offline,setOffline]=useState(!navigator.onLine);var [pull,setPull]=useState(0);var pullY=useRef(0),pulling=useRef(false),refreshing=useRef(false);
 ...
 useEffect(function(){L.openPhoto=function(url,cap){setPhoto({url:url,cap:cap||''});};L.openLink=function(url){setLinkUrl(url);};
  function bip(e){e.preventDefault();window.__luxInstallEv=e;}
  window.addEventListener('beforeinstallprompt',bip);
  return function(){L.openPhoto=null;L.openLink=null;window.removeEventListener('beforeinstallprompt',bip);};},[]);
 /* Разрешения (микрофон/камера/уведомления) — один раз после входа */
 useEffect(function(){if(!me||!me.user)return;var asked='';try{asked=localStorage.getItem('lux-perms-asked')||'';}catch(e){}
  if(!asked){var t=setTimeout(function(){setSheet(function(cur){return cur||{kind:'perms'};});},1400);return function(){clearTimeout(t);};}},[me&&me.user&&me.user.id]);
 ...
 var u=me.user,bks=me.bookmakers||[];var anyDep=bks.some(function(b){return b.deposit;}),anyWd=bks.some(function(b){return b.withdraw;});var recent=(txs||[]).slice(0,3);var fullscreen=['chat','dms','dm','pay','support','bot'].indexOf(page)>=0;var noTop=['chats','contacts','calls','notifs','profile','news','bots','history','tv'].indexOf(page)>=0;
 var body;
 if(page==='home')body=h('div',{className:'page '+dir,key:'home'},
 ...
 else if(page==='contacts')body=h(L.ContactsPage,{key:'contacts',toast:showToast,onBack:back,onOpen:function(id){go('dm',String(id));}});
  ...
  h('div',{className:'store-badges'},
   h('button',{className:'sb',onClick:function(){setSheet({kind:'install',store:'ios'});}},h('svg',{width:22,height:22,viewBox:'0 0 24 24',fill:'currentColor'},h('path',{d:'M17.05 12.54c0-2.42 1.98-3.58 2.07-3.64-1.13-1.65-2.88-1.88-3.5-1.9-1.49-.15-2.9.87-3.66.87-.75 0-1.92-.85-3.16-.83-1.62.02-3.12.94-3.95 2.39-1.69 2.93-.43 7.26 1.21 9.64.8 1.16 1.76 2.47 3.01 2.42 1.21-.05 1.67-.78 3.13-.78 1.46 0 1.87.78 3.15.76 1.3-.02 2.13-1.18 2.92-2.35.92-1.34 1.3-2.64 1.32-2.71-.03-.01-2.53-.97-2.54-3.87ZM14.64 5.03c.67-.8 1.11-1.92.99-3.03-.96.04-2.11.64-2.8 1.44-.61.7-1.15 1.84-1.01 2.93 1.07.08 2.16-.54 2.82-1.34Z'})),h('span',null,h('small',null,'Скачать для'),h('b',null,'iPhone'))),
   h('button',{className:'sb',onClick:function(){setSheet({kind:'install',store:'android'});}},h('svg',{width:22,height:22,viewBox:'0 0 24 24',fill:'currentColor'},h('path',{d:'M3.6 2.2 13.9 12 3.6 21.8c-.3-.2-.6-.6-.6-1.2V3.4c0-.6.3-1 .6-1.2Zm1.5-.6c.2 0 .5.1.7.2l11 6.2-2.6 2.5L5.1 1.6Zm11.7 7 2.8 1.6c.9.5.9 1.9 0 2.4l-2.8 1.6-2.9-2.8 2.9-2.8Zm-1 6.4-11 6.2c-.2.1-.5.2-.7.2l9.1-8.9 2.6 2.5Z'})),h('span',null,h('small',null,'Скачать для'),h('b',null,'Android')))),
 ...
 function renderSheet(sheet,idx){var close=function(){setSheetStack(function(st){return st.slice(0,idx);});};var sh=null;
  if(sheet.kind==='perms')sh=h(L.PermsSheet,{toast:showToast,onClose:close});
  else if(sheet.kind==='install')sh=h(L.InstallSheet,{store:sheet.store,toast:showToast,onClose:close});
  else if(sheet.kind==='deposit')sh=h(L.DepositSheet,{bks:bks,bk:sheet.bk,balance:u.balance,onClose:close,onCreated:function(r){openPay(r.request_id);},onBalance:function(){loadMe();loadTxs('');},onActive:function(tx){showToast('У вас есть активное пополнение','');openPay(tx.id);}});
