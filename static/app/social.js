...
function supMedia(m){
 var k=String(m.kind||'photo').toLowerCase();
 if(k==='voice'||k==='audio')return h('audio',{className:'gm-voice',src:m.file_url,controls:true,preload:'none'});
 if(k==='video'||k==='video_note')return h('video',{className:'gm-video'+(k==='video_note'?' round':''),src:m.file_url,controls:true,preload:'metadata',playsInline:true});
 return h('img',{src:m.file_url,alt:'',onClick:function(){L.openPhoto(m.file_url);}});
}
...
function SupportPage(p){var [msgs,setMsgs]=useState(null);var [busy,setBusy]=useState(false);var box=useRef(null),lastId=useRef(0),alive=useRef(true),got=useRef(false);
 ...
 function send(t,file,extra){setBusy(true);var body;if(file){var ft=String((file&&file.type)||'').toLowerCase();
  var kind=(extra&&extra.duration)?'voice':(ft.indexOf('audio')===0?'voice':(ft.indexOf('video')===0?'video':'photo'));
  var nm=file.name||(kind==='voice'?'voice.webm':kind==='video'?'video.mp4':'photo.jpg');
  body=new FormData();body.append('text',t||'');body.append('kind',kind);body.append('file',file,nm);
  if(extra&&extra.duration)body.append('duration',String(extra.duration));}else body={text:t};return api('/api/web/support/send',{method:'POST',body:body,timeout:60000}).then(function(r){setMsgs(function(prev){return (prev||[]).concat([r.message]);});lastId.current=Math.max(lastId.current,r.message.id);scroll();setBusy(false);return true;}).catch(function(e){p.toast(e.message,'error');setBusy(false);return false;});}
 useVH();return h('div',{className:'gchat support fixed'},h('div',{className:'gc-head'},h('button',{className:'gc-ic',onClick:p.onBack},h(I,{name:'back',size:20})),h('span',{className:'sup-av'},h(I,{name:'headset',size:18})),h('div',{className:'gc-title'},h('b',null,'Поддержка '+(p.brand||'LUXON')),h('small',null,h('i',{className:'dot-on'}),'Оператор онлайн')),h('span',{style:{width:40}})),
  h('div',{className:'gc-msgs',ref:box,onPointerDown:blurComposer},msgs===null?h('div',{className:'center'},h('span',{className:'spin'})):(!msgs.length?h('div',{className:'gc-empty'},h(I,{name:'headset',size:30}),h('b',null,'Напишите нам'),h('span',null,'Ответим в ближайшие минуты')):msgs.map(function(m){return h('div',{key:m.id,className:'gm-wrap'},h('div',{className:'gm '+(m.from==='client'?'mine':'')},h('div',{className:'gm-b'},m.file_url?supMedia(m):null,m.text?h('span',{className:'gm-t'},m.text):null,h('span',{className:'gm-meta'},fmtTime(m.created_at)))));}))),
