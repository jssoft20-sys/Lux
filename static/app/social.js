...
function UserSheet(p){var [u,setU]=useState(p.user||null);var [tab,setTab]=useState('info');var [media,setMedia]=useState(null);var [inCt,setInCt]=useState(null);var [shareU,setShareU]=useState(false);var [askAlias,setAskAlias]=useState(null);
 ...
 var bodyTab=tab==='qr'?h('div',{className:'uqr'},h('img',{src:'/api/web/users/'+encodeURIComponent(u.username||('id'+u.id))+'/qr.png',alt:'QR',onError:function(e){e.target.style.display='none';}}),h('div',{className:'linkbox',onClick:function(){copyText(ulink(u),'Ссылка скопирована');}},h(I,{name:'link',size:16}),h('span',null,ulink(u).replace(/^https?:\/\//,'')),h(I,{name:'copy',size:15}))):(tab==='info'?h(React.Fragment,null,
   ...
   /* Как в ТГ: ссылка на профиль, QR и пересылка в чат */
   h('div',{className:'u-actions'},
    inCt!==null?h('button',{onClick:togCt},h(I,{name:inCt?'check':'user',size:18}),inCt?'В контактах':'В контакты'):null,
    h('button',{onClick:function(){copyText(ulink(u),'Ссылка на профиль скопирована');}},h(I,{name:'link',size:18}),'Ссылка'),
