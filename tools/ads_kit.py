...
# ── Валюта аккаунта ──────────────────────────────────────────────────────────
# Google Ads не поддерживает кыргызский сом: аккаунт для Кыргызстана заводится
# в долларах, и после создания валюту сменить нельзя. Поэтому планируем в сомах,
# а в файлы выгружаем суммы в валюте аккаунта. Курс сильно сдвинулся — поменяйте
# KGS_PER_USD и перегенерируйте файлы.
CURRENCY = 'USD'        # 'USD' или 'KGS'
KGS_PER_USD = 87.45     # курс на 07.09.2026
BUDGET_KGS = 1200       # сом/день — сколько реально хотим тратить
MAX_CPC_KGS = 22        # сом — потолок цены клика на старте

def money(kgs):
    """Сумма в валюте аккаунта."""
    return kgs if CURRENCY == 'KGS' else round(kgs / KGS_PER_USD, 2)

def fmt(v):
    """Сумма с обозначением валюты, для текста."""
    return f'${v:,.2f}' if CURRENCY == 'USD' else f'{v:g} сом'

BUDGET_MAIN = money(BUDGET_KGS)
MAX_CPC = money(MAX_CPC_KGS)
...
GROUPS = [
    ...
    {
        'name': '5 Кыргызча машина жана кочуу',
...
| Параметр | Значение |
|---|---|
| Валюта аккаунта | **{CURRENCY}** — сом Google Ads не поддерживает |
| Курс пересчёта | 1 $ = {KGS_PER_USD} сом |
| Бюджет в день | **{fmt(BUDGET_MAIN)}** (это {BUDGET_KGS} сом) |
| Предел цены клика | **{fmt(MAX_CPC)}** (это {MAX_CPC_KGS} сом) |
| Стратегия | Максимум кликов с пределом цены клика |
| Широкое соответствие | {broad} фраз для охвата, остальные фразовые |

> **Все суммы в интерфейсе вводятся в валюте аккаунта.** Введёте 1200 вместо {fmt(BUDGET_MAIN)} —
> получите {fmt(BUDGET_KGS)} в день, это {BUDGET_KGS * (1 if CURRENCY == 'KGS' else KGS_PER_USD):,.0f} сом.
...
1. **Конверсии до рекламы.** Цели → Конверсии → три действия: WhatsApp ({fmt(money(300))}),
   Звонок ({fmt(money(300))}), Заявка из калькулятора ({fmt(money(500))}) — ценность указывается
...
- **Цена заявки**: норма {fmt(money(150))}–{fmt(money(400))} (150–400 сом). Дороже {fmt(money(600))}
