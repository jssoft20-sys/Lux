...
function setToken(value) {
  ...
  // Внутри приложения курьера токен нужен ещё и телефону: его служба говорит
  // с сервером сама, когда экран погас, и без токена ей нечем представиться.
  // Токен меняется только здесь, поэтому и сообщаем отсюда — иначе пришлось бы
  // помнить об этом на каждом экране входа и выхода.
  native.setToken(memToken);
  return memToken;
}
