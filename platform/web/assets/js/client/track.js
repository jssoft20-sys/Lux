...
export function mountTrack(app, pid, token) {
  ...
  /* Ссылка на заказ: /go/share/AB12CD?v=токен. Токена отслеживания в ней нет,
     и это главное: подмешиваем только токен просмотра, которым заказ можно
     смотреть, но нельзя ни отменить, ни оценить — на любое действие им сервер
     отвечает 403. По такой ссылке человек видит живую карту с машиной, статус и
     примерное время, а телефон, квартиру и точный адрес сервер из ответа
     вырезает сам (см. server/routers/public.py, режим «view»). */
  function shareLink() {
    const order = store.get().order || {};
    const view = String(order.view_token || '');
    let url = siteUrl('share/' + encodeURIComponent(pid));
    if (/^[A-Za-z0-9_-]{16,64}$/.test(view)) url += '?v=' + view;
    haptic();
    if (navigator.share) {
      navigator.share({
