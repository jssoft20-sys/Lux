...
STRAIGHT_TTL = 20              # прямая линия — это «не дозвонились», а не маршрут
...
def route(points):
    ...
    if not out:
        ...
        # Прямую линию держим в памяти считаные секунды, а не десять минут.
        # Это запасной ответ на случай, когда маршрутизатор не отозвался: один
        # его сбой не должен превращать нитку маршрута в отрезок на весь этот
        # срок — следующая же попытка возьмёт настоящую дорогу.
        ttl = (TRAFFIC_TTL if out['traffic']
               else (STRAIGHT_TTL if out['provider'] == 'straight' else ROUTE_TTL))
        _route_cache.put(key, out, ttl)
