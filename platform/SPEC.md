...
## 9. Оплата брони, бонусы и карточка ссылки (версия 3)

### Бронь вместо полной оплаты
Клиент платит вперёд **только комиссию сервиса** — это подтверждение, что человек
настоящий. Остальное он отдаёт курьеру наличными. Размер брони считается из
комиссии и зажимается между `payment.prepay_min` и `payment.prepay_max`.

### Оптима Банк
Владелец вводит в админке **ровно два поля**: `payment.optima_key` (X-API-KEY) и
`payment.optima_company` (legalPartyId). Торговая точка и касса определяются сами
через `GET /api/v2/get-sale-point-infos/{legalPartyId}` и ложатся в
`payment.optima_sale_point` и `payment.optima_cash`.

```
POST /pay/{public_id}/qr?t=токен    создать или вернуть QR заказа
GET  /pay/{public_id}/status?t=     состояние оплаты, опрос не чаще раза в 2 с
POST /pay/callback                  уведомление банка, Basic Auth, ответ 200 обязателен
GET  /admin/pay/sale-points         точки и кассы из банка
PUT  /admin/pay/settings            ключ, ID компании, выбранная точка
POST /admin/pay/test                проверка связи
```
Суммы в банк уходят **в сомах с двумя знаками**, внутри системы — тыйыны.
Обработка уведомления идемпотентна: повторное по тому же `transactionId` не платит дважды.

### Бонусы (`server/bonus.py`)
Кэшбек `bonus.percent` с **завершённого** заказа; списать можно не больше
`bonus.max_share` процентов заказа; сгорают через `bonus.expire_days` без заказов;
приглашение друга — `bonus.invite_friend` другу и `bonus.invite_owner` пригласившему
после первого **выполненного** заказа друга. Каждое движение — строка в журнале с причиной.

### От двери до двери
Позиция расчёта `door_to_door`: надбавка `price.door_to_door` за **каждую** точку,
где переключатель включён. Курьер поднимается к двери, а не ждёт у машины.

### Карточка для ссылки (`server/share.py`)
```
GET /share/{public_id}.png   картинка 1200×630, рисуется вручную через zlib
GET /share/{public_id}       страница с Open Graph
```
Токен отслеживания в них не участвует. Показываем только безобидное: статус, город,
примерное время. Ни телефонов, ни квартир, ни имени клиента.

## 10. Версия 4: ссылка, приложение, оболочка

### Два токена у заказа
- `track_token` — у того, кто заказывал. Видит всё, может отменить, оценить, писать курьеру.
- `view_token` — в ссылке, которой делятся. Видит движение машины и статус; телефона,
  квартиры, домофона и имени в ответе нет, номер дома в адресе срезан, поле
  `readonly: true`. События `message` и `message_read` в такой поток не уходят вовсе.
  Любое действие с этим токеном — 403.

Ссылка: `/share/{public_id}?v={view_token}`. Робот мессенджера приходит без токена
и получает прежнюю карточку с Open Graph; человек с токеном — живую карту.

### Демо-оплата
`payment.demo` — экран оплаты работает целиком, QR рисует сам сервис
(`server/qrcode.py`), деньги никуда не идут. Нужен, чтобы владелец увидел то же,
что увидит клиент, ещё до того, как банк выдаст ключи.
```
POST /admin/pay/demo           создать пример заказа с кодом
POST /admin/pay/demo/confirm   отметить его оплаченным
```

### Оболочка
- Светлая тема по умолчанию. Тёмная работает целиком, но открывается светлая.
- `sheet({ expandable: true })` — шторка тянется вверх до доступной высоты и вниз
  до закрытия, доезжает до ближайшего положения сама.
- `sheet({ full: true })` — сразу во весь экран, прокрутка только внутри.
- `pullToRefresh(scrollEl, onRefresh)` — потянуть сверху и обновить. Не срабатывает,
  когда открыта шторка или прокрутка не в нуле.
- `router.overlay(name)` / `router.closeOverlay()` — открытая шторка попадает в адрес
  (`#/…/~profile`), перезагрузка возвращает человека туда же.
- Заставка в html, гасит её `core/shell.js`. Полностью показывается только при
  холодном запуске, при обычной перезагрузке не мелькает.

### Приложение курьера
`android/` — проект на Java без сторонних библиотек. WebView вокруг `/go/courier`
плюс служба на переднем плане: координаты раз в 5-10 секунд, поток предложений
через SSE, уведомления со звуком. Без Firebase. Собранный файл лежит в
`web/app/sprintergo-courier.apk` и отдаётся по ссылке — водитель ставит его с
телефона, без Google Play.
