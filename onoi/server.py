...
# Префикс URL, под которым приложение смонтировано за nginx ("" = без префикса).
PREFIX = os.environ.get("ONOI_URL_PREFIX", "/onoipay").strip()
...
def _url(path: str) -> str:
    """Абсолютный путь с префиксом для всего, что сервер отдаёт наружу (push, qr_photo_url, logo, receipt)."""
    p = str(path or "")
    if not p or p.startswith(("http://", "https://", "data:", "blob:")):
        return p
    if not p.startswith("/"):
        p = "/" + p
...
def _app_path(request: Request) -> str:
    """Путь запроса относительно приложения (без root_path/префикса)."""
...
app = FastAPI(title="OnoiPay Admin", docs_url=None, redoc_url=None, openapi_url=None)
...
def health():
    ...
    return {
        ...
        "brand": BRAND,
        "prefix": PREFIX,
        "app_version": (_admin_version() if "_admin_version" in globals() else "") or (_LUX_APP_VERSION if "_LUX_APP_VERSION" in globals() else ""),
...
def get_session(request: Request, *, allow_locked: bool = False, touch: bool = True) -> dict[str, Any]:
    token = request.cookies.get("onoipay_session")
...
async def login(request: Request):
    ...
    res.set_cookie(
        "onoipay_session", token, httponly=True,
...
async def logout(request: Request):
    ...
    res.delete_cookie("onoipay_session")
    return res
...
# === ONOIPAY v10.31 FAST UI DATA LAYER ===
@contextmanager
def _ui_read_conn():
...
def _ui_base_payload():
    ...
    settings["support_username"] = str(cfg.get("main_bot", {}).get("support_username", ""))
    ...
    return {
        ...
        "bot_config": [
            {"name": "OnoiPay", "enabled": bool(cfg.get("main_bot", {}).get("enabled", True))},
...
async def _localize_support_media(remote_url: str, chat_id: int, kind: str = "photo") -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            r = await client.get(value, headers={"Accept": "*/*", "User-Agent": "OnoiPay/1.0"})
            r.raise_for_status()
            raw = r.content
...
async def _localize_support_photo(remote_url: str, chat_id: int) -> str:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            r = await client.get(value, headers={"Accept":"image/*","User-Agent": "OnoiPay/1.0"})
            r.raise_for_status()
            raw = r.content
...
async def bootstrap(request: Request):
    ...
    data["bot_config"] = [{"name": "OnoiPay", "enabled": bool(reload_config().get("main_bot", {}).get("enabled", True))}, {"name": "Поддержка", "enabled": bool(reload_config().get("support_bot", {}).get("enabled", True))}]
    return integrated_bootstrap(data)



# === ONOIPAY FAST LIVE REVISION v1 ===
...
def _validate_image(raw: bytes, max_side: int = 2400, quality: int = 92) -> tuple[bytes, str]:
...
async def upload_receipt(tx_id: str, request: Request, file: UploadFile = File(...)):
    ...
    try:
        raw, suffix = await asyncio.to_thread(_validate_image, raw, 2400, 92)
...
async def broadcast(request: Request):
    ...
    bot_label = str(data.get("bot") or "OnoiPay")
...
async def terminate_device(device_id: str, request: Request):
    sess = get_session(request)
    current_token = request.cookies.get("onoipay_session", "")
...
# ===== OnoiPay v9 integrated bot / MacroDroid / provider API =====
...
# === ONOIPAY QUIET TRANSACTION LOGS v1 ===
...
DB_FILE = STORAGE / "onoipay.sqlite3"
CONFIG_MTIME = 0.0
...
def _db_init():
    ...
    with _DB_LOCK, _db_conn() as c:
        ...
        _ensure_column(c, "macro_events", "source_hint", "TEXT DEFAULT 'onoipay'")
...
def _public_requisite(row: dict) -> dict:
    ...
    return {
        ...
        "imap_folder": str(row.get("imap_folder") or ""),
        "mail_enabled": bool(row.get("mail_enabled", True)),
        "mail_state": _demir_mail_state_safe(row.get("id")),
        "logo_url": str(row.get("logo_url") or ""),
        "created_at": row.get("created_at") or "",
    }


def _demir_mail_state_safe(rid: Any) -> dict:
    """Статус почтового воркера. Блок почты объявлен ниже по файлу,
    поэтому обращаемся к нему мягко — до его загрузки отдаём пустой словарь."""
    try:
        return _demir_mail_status_for(str(rid or ""))
    except Exception:
        return {}
...
# === ONOIPAY SUPPORT QUEUE v1 ===
# === ONOIPAY SUPPORT QUEUE ISSUE RESET v2 ===
...
def integrated_bootstrap(data: dict) -> dict:
    ...
    st["support_username"] = str(cfg.get("main_bot", {}).get("support_username", ""))
...
def apply_ui_settings_to_config(data: dict) -> None:
    ...
    if "support_username" in data:
        val=str(data.get('support_username') or '').strip()
...
def _tx_client_names(force: bool = False) -> dict[int, str]:
    """chat_id -> имя клиента из bot_users. Кэш на 30 секунд: список заявок
...
def _tx_to_front(row):
    ...
    _error_code = str(row["error_code"] or "").strip().lower() if "error_code" in row.keys() else ""
    attention = ""
    attention_reason = ""
    if ui_status == "problem":
        attention_reason = error_text or "Причина не указана"
        if kind == "deposit":
            if _error_code == "currency_mismatch":
                attention = "Валюта аккаунта не совпадает — нужен новый ID"
            else:
                attention = "Надо пополнить: деньги пришли, букмекер не зачислил"
        else:
            attention = "Надо проверить вывод: букмекер не вернул сумму"
    return {
        "attention": attention, "attention_reason": attention_reason, "error_code": _error_code,
        ...
        "account_label": _tx_account, "from_site": False,
...
def _http_json(url, method="GET", data=None, headers=None):
    ...
    h = {"Content-Type": "application/json", "User-Agent": "OnoiPay/1.0"}
    h.update(headers or {})
...
_LUX_QR_POOL = ThreadPoolExecutor(max_workers=2, thread_name_prefix="onoi-qr")
...
def _read_qr_image_source(source: str) -> tuple[bytes, str]:
    ...
    req = _urlreq.Request(value, headers={"User-Agent": "OnoiPay/1.0", "Accept": "image/*"})
...
def _main_success_text(row: dict, processing_seconds: int = 1) -> str:
    ...
    if dep:
        ...
        return (
            ...
            f"⚡ Зачислено автоматически за {max(1, int(processing_seconds or 1))} сек"
        )
...
def _queue_main_success_replace(row: dict, processing_seconds: int = 1) -> bool:
    ...
    try:
        ...
        main_db = BASE / "data" / "onoi.sqlite3"
        if main_db.exists():
...
threading.Thread(target=_expiry_worker, daemon=True, name="onoi-expiry").start()
...
_LUX_WQR_POOL = ThreadPoolExecutor(max_workers=1, thread_name_prefix="onoi-wqr")
...
threading.Thread(target=_lux_withdraw_qr_worker, daemon=True, name="onoi-withdraw-qr").start()
...
async def ui_alerts(request: Request):
    ...
    return {"ok": True, "app_version": _admin_version() or _LUX_APP_VERSION, "alerts": alerts, "server_time": now_iso()}


threading.Thread(target=_lux_cash_monitor, daemon=True, name="onoi-cashdesk").start()
...
# === ONOIPAY SERVCUL DUPLICATE RECOVERY v2 ===
...
_MACRO_EXECUTOR = ThreadPoolExecutor(max_workers=_MACRO_WORKERS, thread_name_prefix="onoi-macro")
...
threading.Thread(target=_macro_queue_worker, daemon=True, name="onoi-macro-queue").start()
...
async def _macro_process(request: Request, source_hint: str = "onoipay"):
    if _payment_check_mode() != "macro":
...
async def macro_webhook_keyed(webhook_key: str, request: Request):
    _macro_webhook_auth(request, webhook_key)
    return await _macro_process(request, "api")


@app.api_route("/notifications/{webhook_key}", methods=["GET", "POST"])
@app.api_route("/notifications/{webhook_key}/", methods=["GET", "POST"])
@app.api_route("/onoi/notifications/{webhook_key}", methods=["GET", "POST"])
@app.api_route("/onoi/notifications/{webhook_key}/", methods=["GET", "POST"])
async def onoi_notifications_keyed(webhook_key: str, request: Request):
    _macro_webhook_auth(request, webhook_key)
    return await _macro_process(request, "onoipay")


@app.api_route("/notifications", methods=["GET", "POST"])
@app.api_route("/notifications/", methods=["GET", "POST"])
@app.api_route("/onoi/notifications", methods=["GET", "POST"])
@app.api_route("/onoi/notifications/", methods=["GET", "POST"])
async def onoi_notifications(request: Request):
    _macro_webhook_auth(request)
    return await _macro_process(request, "onoipay")


@app.get("/notifications-test")
@app.get("/onoi/notifications-test")
async def onoi_notifications_test():
    return {
        "ok": True,
        "service": "OnoiPay MacroDroid",
        "version": str(reload_config().get("version") or "1.0.0"),
        "post_url": _url("/notifications/<webhook_key>"),
        "auth": "ключ в пути, заголовке x-macro-key или ?key=",
        "accepted": ["text/plain", "application/json", "application/x-www-form-urlencoded", "GET query"],
    }


# === LUX DEMIR MAIL PAYMENTS (10.46) =========================================
# Demir не отдаёт API оплаты, но шлёт письмо о каждом зачислении на ящик
# реквизита. На ящик поднимаются ДВЕ независимые IMAP-сессии:
#   role=idle — держит IMAP IDLE, сервер сам будит нас в момент доставки письма;
#   role=poll — страховка, дёргает UID SEARCH раз в poll_seconds, а после любой
#               активности переходит на fast_poll_seconds на burst_seconds.
# Обе сессии пишут в одну очередь macro_events, дальше работает штатный матчинг.
# Повторное зачисление невозможно на трёх уровнях: клейм Message-ID в памяти,
# UNIQUE(event_hash) в macro_events и перевод заявки pending->crediting одним
# UPDATE с проверкой rowcount в _macro_try_process_event.
...
_DEMIR_MAIL_DEFAULTS: dict[str, Any] = {
    "enabled": True,
...
_DEMIR_MAIL_STATUS: dict[str, dict[str, Any]] = {}
_DEMIR_MAIL_THREADS: dict[str, threading.Thread] = {}
_DEMIR_MAIL_LOCK = threading.RLock()
_DEMIR_MAIL_CLAIMS: dict[str, float] = {}
_DEMIR_MAIL_ROLES = ("idle", "poll")


def _demir_mail_cfg(cfg: dict | None = None) -> dict[str, Any]:
    cfg = cfg or reload_config()
    raw = cfg.get("demir_mail")
    out = dict(_DEMIR_MAIL_DEFAULTS)
    if isinstance(raw, dict):
    ...
    try:
        ...
        out["imap_port"] = 993
    out["imap_ssl"] = bool(out.get("imap_ssl", True))
    try:
        out["watchers"] = max(1, min(4, int(out.get("watchers") or 2)))
    except Exception:
        out["watchers"] = 2
    # Нижние границы держат нагрузку на почтовый кластер в рамках лимитов
    # провайдера: чаще 5 раз в секунду Timeweb начинает резать соединения.
    out["poll_seconds"] = max(0.2, min(60.0, float(out.get("poll_seconds") or 1.0)))
    out["fast_poll_seconds"] = max(0.05, min(out["poll_seconds"], float(out.get("fast_poll_seconds") or 0.2)))
    out["burst_seconds"] = max(0.0, min(120.0, float(out.get("burst_seconds") or 6.0)))
    out["idle_timeout_seconds"] = max(30, min(1500, int(out.get("idle_timeout_seconds") or 120)))
    out["lookback_minutes"] = max(1, min(1440, int(out.get("lookback_minutes") or 45)))
    out["reconnect_seconds"] = max(2, min(300, int(out.get("reconnect_seconds") or 5)))
    for key in ("senders", "subject_hints", "bank_types"):
        value = out.get(key)
        if isinstance(value, str):
            value = [x.strip() for x in value.split(",")]
        out[key] = [str(x).strip().lower() for x in (value or []) if str(x).strip()]
    return out


def _demir_mail_account(row: dict, mail_cfg: dict[str, Any]) -> dict[str, Any] | None:
    """Реквизит → параметры IMAP. None, если почта для него не настроена."""
    if not isinstance(row, dict):
        return None
    if not row.get("enabled", True):
        return None
    bank_type = str(row.get("bank_type") or "").strip().lower()
    allowed = mail_cfg.get("bank_types") or ["demir"]
    if bank_type not in allowed and not bool(row.get("mail_enabled")):
        return None
    if row.get("mail_enabled") is False:
        return None
    login = str(row.get("email") or "").strip()
    password = str(row.get("mail_password") or "")
    if not login or not password or password == "••••••":
        return None
    host = str(row.get("imap_host") or mail_cfg["imap_host"]).strip()
    try:
        port = int(row.get("imap_port") or mail_cfg["imap_port"])
    except Exception:
        port = int(mail_cfg["imap_port"])
    return {
        "requisite_id": str(row.get("id") or ""),
        "name": str(row.get("name") or "Реквизит"),
        "login": login,
        "password": password,
        "host": host,
        "port": port,
        "ssl": bool(row.get("imap_ssl", mail_cfg["imap_ssl"])),
        "folder": str(row.get("imap_folder") or mail_cfg["folder"] or "INBOX").strip() or "INBOX",
        "fingerprint": f"{login}|{host}|{port}|{hashlib.sha256(password.encode()).hexdigest()[:12]}",
    }


def _demir_mail_accounts(cfg: dict | None = None) -> list[dict[str, Any]]:
    cfg = cfg or reload_config()
    mail_cfg = _demir_mail_cfg(cfg)
    if not bool(mail_cfg.get("enabled", True)):
        return []
    out: list[dict[str, Any]] = []
    for row in (cfg.get("macro") or {}).get("requisites", []) or []:
        acc = _demir_mail_account(row, mail_cfg)
        if acc:
            out.append(acc)
    return out


def _demir_mail_state(rid: str) -> dict[str, Any]:
    return _DEMIR_MAIL_STATUS.setdefault(
        str(rid or ""),
        {"events": 0, "duplicates": 0, "skipped": 0, "roles": {}},
    )


def _demir_mail_set_status(rid: str, role: str = "", **fields: Any) -> None:
    with _DEMIR_MAIL_LOCK:
        state = _demir_mail_state(rid)
        target = state["roles"].setdefault(role, {}) if role else state
        target.update(fields)
        target["updated_at"] = now_iso()
        state["updated_at"] = now_iso()


def _demir_mail_bump(rid: str, key: str) -> None:
    with _DEMIR_MAIL_LOCK:
        state = _demir_mail_state(rid)
...
def _demir_mail_status_for(rid: str) -> dict[str, Any]:
    with _DEMIR_MAIL_LOCK:
        return deepcopy(_DEMIR_MAIL_STATUS.get(str(rid or ""), {}))


def _demir_mail_claim(key: str) -> bool:
    """Одно письмо обрабатывает ровно одна сессия. Клеймы стареют за час,
    чтобы память не росла, а дубли всё равно ловились по event_hash в БД."""
    stamp = time.monotonic()
    with _DEMIR_MAIL_LOCK:
        if key in _DEMIR_MAIL_CLAIMS:
            return False
        if len(_DEMIR_MAIL_CLAIMS) > 4000:
            for old_key, old_stamp in list(_DEMIR_MAIL_CLAIMS.items()):
                if stamp - old_stamp > 3600:
                    _DEMIR_MAIL_CLAIMS.pop(old_key, None)
        _DEMIR_MAIL_CLAIMS[key] = stamp
        return True


def _demir_mail_unclaim(key: str) -> None:
    with _DEMIR_MAIL_LOCK:
        _DEMIR_MAIL_CLAIMS.pop(key, None)


def _demir_mail_header(value: Any) -> str:
    if value in (None, ""):
        return ""
    try:
        parts = _email_decode_header(str(value))
    except Exception:
        return str(value)
    out: list[str] = []
...
def _demir_mail_body(msg) -> str:
    """Текст письма: text/plain и text/html без тегов."""
    plain: list[str] = []
    html: list[str] = []
    try:
    ...
    # Банк может слать «пустую» текстовую часть, а сумму — только в html,
    # поэтому берём обе части: сначала plain, следом html без тегов.
    chunks = list(plain)
    if html:
        stripped = _re.sub(r"<[^>]+>", " ", "\n".join(html))
...
def _demir_mail_amount(text: str, mail_cfg: dict[str, Any]) -> float:
    pattern = str(mail_cfg.get("amount_regex") or "").strip()
    if pattern:
        match = _re.search(pattern, text, flags=_re.I | _re.S)
        if not match:
            raise ValueError("amount_regex не совпал")
        raw = match.group(1) if match.groups() else match.group(0)
        value = float(_Decimal(str(raw).replace(" ", "").replace("\u00a0", "").replace(",", ".")).quantize(_Decimal("0.01")))
        if value <= 0:
            raise ValueError("Сумма <= 0")
        return value
    return _extract_macro_amount(text)


def _demir_mail_accept(msg, subject: str, mail_cfg: dict[str, Any]) -> str:
    """'' — письмо годится, иначе причина пропуска."""
    senders = mail_cfg.get("senders") or []
    if senders:
        sender = (str(msg.get("From") or "") + " " + str(msg.get("Return-Path") or "")).lower()
        if not any(x in sender for x in senders):
            return "sender_skipped"
    hints = mail_cfg.get("subject_hints") or []
    if hints:
        low = subject.lower()
        if not any(x in low for x in hints):
            return "subject_skipped"
    return ""


def _demir_mail_sent_at(msg) -> datetime | None:
    raw = msg.get("Date")
    if not raw:
        return None
    try:
        sent = _email_parsedate(str(raw))
    except Exception:
        return None
    if sent is None:
        return None
    return sent.replace(tzinfo=TZ) if sent.tzinfo is None else sent


def _demir_mail_fresh(msg, lookback_minutes: int) -> bool:
    sent = _demir_mail_sent_at(msg)
    if sent is None:
        return True
    delta = (now() - sent).total_seconds()
    return -300 <= delta <= lookback_minutes * 60


def _demir_mail_submit(digest: str, amount: float, raw_text: str, source_hint: str) -> dict[str, Any]:
    """Кладём событие в ту же очередь, что и вебхук MacroDroid.
    UNIQUE(event_hash) — последний барьер против повторного зачисления:
    гонку двух сессий ловим на INSERT, а не на предварительном SELECT."""
    try:
        with _DB_LOCK, _db_conn() as c:
            existing = c.execute("SELECT id,status FROM macro_events WHERE event_hash=?", (digest,)).fetchone()
            if existing:
                return {"duplicate": True, "event_id": int(existing["id"]), "status": str(existing["status"] or "")}
            cur = c.execute(
                "INSERT INTO macro_events(event_hash,amount,raw_text,status,created_at,updated_at,attempts,source_hint) VALUES(?,?,?,'pending',?,?,0,?)",
                (digest, float(amount), raw_text[:5000], now_iso(), now_iso(), source_hint),
            )
            event_id = int(cur.lastrowid)
    except _sqlite3.IntegrityError:
        return {"duplicate": True, "event_id": 0, "status": "race"}
    _macro_submit_event(event_id)
    return {"duplicate": False, "event_id": event_id, "status": "pending"}


def _demir_mail_connect(acc: dict[str, Any], timeout: float = 25.0):
    if acc.get("ssl", True):
        conn = _imaplib.IMAP4_SSL(acc["host"], int(acc["port"]), timeout=timeout)
    else:
        conn = _imaplib.IMAP4(acc["host"], int(acc["port"]), timeout=timeout)
        try:
            conn.starttls()
        except Exception:
            pass
    conn.login(acc["login"], acc["password"])
    conn.select(f'"{acc["folder"]}"')
    return conn


def _demir_mail_idle(conn, timeout: float) -> bool:
    """IMAP IDLE вручную (imaplib его не умеет). True — пришло новое письмо."""
    tag = conn._new_tag()
    conn.send(b"%s IDLE\r\n" % tag)
    line = conn.readline()
    if not line.startswith(b"+"):
        raise RuntimeError(f"IDLE отклонён: {line[:80]!r}")
    changed = False
    ...
    try:
        while True:
            ...
            if remain <= 0:
                break
            ready, _, _ = _select_mod.select([sock], [], [], min(remain, 5.0))
            if not ready:
                continue
            line = conn.readline()
            if not line:
                raise RuntimeError("IMAP-соединение закрыто в IDLE")
            upper = line.upper()
...
def _demir_mail_fetch_uids(conn, last_uid: int, since: str) -> list[int]:
    if last_uid > 0:
...
def _demir_mail_handle_uid(conn, acc: dict[str, Any], uid: int, mail_cfg: dict[str, Any]) -> dict[str, Any]:
    typ, data = conn.uid("FETCH", str(uid), "(BODY.PEEK[])")
    if typ != "OK" or not data or not isinstance(data[0], tuple):
        return {"skipped": "fetch_failed"}
    msg = _email_mod.message_from_bytes(data[0][1])
    subject = _demir_mail_header(msg.get("Subject"))
    reason = _demir_mail_accept(msg, subject, mail_cfg)
    if reason:
        return {"skipped": reason, "subject": subject}
    if not _demir_mail_fresh(msg, int(mail_cfg["lookback_minutes"])):
        return {"skipped": "too_old", "subject": subject}
    text = (subject + "\n" + _demir_mail_body(msg)).strip()
    try:
        amount = _demir_mail_amount(text, mail_cfg)
    except ValueError as exc:
        return {"skipped": "no_amount", "subject": subject, "error": str(exc)}
    message_id = str(msg.get("Message-ID") or "").strip() or f"uid:{uid}:{msg.get('Date') or ''}"
    # Хеш строится только по ящику и Message-ID: одно письмо = максимум одно
    # зачисление, даже если сумма будет разобрана иначе после смены regex.
    key = f"demir_mail|{acc['login']}|{message_id}"
    digest = hashlib.sha256(key.encode()).hexdigest()
    if not _demir_mail_claim(key):
        return {"duplicate": True, "subject": subject, "amount": amount, "claimed": True}
    try:
        result = _demir_mail_submit(digest, amount, text, "demir_mail")
    except Exception:
        _demir_mail_unclaim(key)
        raise
    sent = _demir_mail_sent_at(msg)
    result["amount"] = amount
    result["subject"] = subject
    result["mail_lag"] = round((now() - sent).total_seconds(), 2) if sent else None
    if mail_cfg.get("mark_seen", True):
        try:
...
def _demir_mail_scan(conn, acc: dict[str, Any], mail_cfg: dict[str, Any], last_uid: int, since: str, role: str) -> tuple[int, int]:
    """Разбирает новые письма. Возвращает (новый last_uid, сколько зачтено)."""
    rid = acc["requisite_id"]
    hits = 0
    for uid in _demir_mail_fetch_uids(conn, last_uid, since):
        last_uid = max(last_uid, uid)
        started = time.monotonic()
        try:
            res = _demir_mail_handle_uid(conn, acc, uid, mail_cfg)
        except Exception as exc:
            print(f"[DEMIRMAIL] uid_error requisite={rid} role={role} uid={uid} error={exc}", flush=True)
            continue
        if res.get("skipped"):
            _demir_mail_bump(rid, "skipped")
            continue
        if res.get("duplicate"):
            _demir_mail_bump(rid, "duplicates")
            continue
        hits += 1
        _demir_mail_bump(rid, "events")
        took_ms = int((time.monotonic() - started) * 1000)
        _demir_mail_set_status(
            rid,
            last_amount=float(res.get("amount") or 0),
            last_event_at=now_iso(),
            last_event_role=role,
            last_subject=str(res.get("subject") or "")[:160],
            last_handle_ms=took_ms,
            last_mail_lag_seconds=res.get("mail_lag"),
        )
        add_log(
            "Письмо Demir принято",
            f"{acc['name']} • {float(res.get('amount') or 0):.2f} сом • {took_ms} мс",
            "info", float(res.get("amount") or 0), kind="deposit", ip="System",
        )
        print(
            f"[DEMIRMAIL] event requisite={rid} role={role} amount={float(res.get('amount') or 0):.2f} "
            f"event={res.get('event_id')} handle_ms={took_ms} mail_lag={res.get('mail_lag')}s",
            flush=True,
        )
    return last_uid, hits


def _demir_mail_worker(rid: str, role: str) -> None:
    """role=idle — пуш через IMAP IDLE; role=poll — независимая сессия-страховка."""
    idle_failures = 0
    while True:
        cfg = reload_config()
        mail_cfg = _demir_mail_cfg(cfg)
        acc = next((x for x in _demir_mail_accounts(cfg) if x["requisite_id"] == rid), None)
        stop = acc is None or (role == "poll" and int(mail_cfg["watchers"]) < 2)
        if stop:
            _demir_mail_set_status(rid, role, connected=False, state="stopped", error="")
            with _DEMIR_MAIL_LOCK:
                _DEMIR_MAIL_THREADS.pop(f"{rid}:{role}", None)
            print(f"[DEMIRMAIL] stopped requisite={rid} role={role}", flush=True)
            return
        fingerprint = acc["fingerprint"]
        conn = None
        try:
            conn = _demir_mail_connect(acc)
            last_uid = 0
            burst_until = 0.0
            _demir_mail_set_status(
                rid, role, connected=True, state="connected", error="", login=acc["login"],
                host=acc["host"], port=acc["port"], folder=acc["folder"], connected_at=now_iso(),
            )
            print(f"[DEMIRMAIL] connected requisite={rid} role={role} login={acc['login']}", flush=True)
            since = (now() - timedelta(minutes=int(mail_cfg["lookback_minutes"]) + 60)).strftime("%d-%b-%Y")
            while True:
                cfg = reload_config()
                mail_cfg = _demir_mail_cfg(cfg)
                fresh = next((x for x in _demir_mail_accounts(cfg) if x["requisite_id"] == rid), None)
                if not fresh or fresh["fingerprint"] != fingerprint:
                    break
                if role == "poll" and int(mail_cfg["watchers"]) < 2:
                    break
                if _payment_check_mode() != "macro":
                    _demir_mail_set_status(rid, role, state="idle_mode_optima")
                    time.sleep(2.0)
                    continue
                last_uid, hits = _demir_mail_scan(conn, acc, mail_cfg, last_uid, since, role)
                if hits:
                    burst_until = time.monotonic() + float(mail_cfg["burst_seconds"])
                use_idle = (
                    role == "idle"
                    and bool(mail_cfg.get("idle_enabled", True))
                    and idle_failures < 2
                    and time.monotonic() >= burst_until
                )
                if use_idle:
                    try:
                        _demir_mail_set_status(rid, role, state="idle")
                        _demir_mail_idle(conn, float(mail_cfg["idle_timeout_seconds"]))
                        continue
                    except Exception as exc:
                        idle_failures += 1
                        print(f"[DEMIRMAIL] idle_failed requisite={rid} attempt={idle_failures} error={exc}", flush=True)
                        raise
                fast = time.monotonic() < burst_until
                _demir_mail_set_status(rid, role, state="burst" if fast else "polling")
                conn.noop()
                time.sleep(float(mail_cfg["fast_poll_seconds"] if fast else mail_cfg["poll_seconds"]))
        except Exception as exc:
            _demir_mail_set_status(rid, role, connected=False, state="error", error=str(exc)[:200], error_at=now_iso())
            print(f"[DEMIRMAIL] error requisite={rid} role={role} error={exc}", flush=True)
        finally:
            try:
                if conn is not None:
                    try:
                ...
                pass
        time.sleep(float(_demir_mail_cfg()["reconnect_seconds"]))


def _demir_mail_supervisor() -> None:
    while True:
        try:
            cfg = reload_config()
            mail_cfg = _demir_mail_cfg(cfg)
            watchers = int(mail_cfg["watchers"])
            for acc in _demir_mail_accounts(cfg):
                rid = acc["requisite_id"]
                roles = _DEMIR_MAIL_ROLES[:1] if watchers < 2 else _DEMIR_MAIL_ROLES
                for role in roles:
                    key = f"{rid}:{role}"
                    with _DEMIR_MAIL_LOCK:
                        thread = _DEMIR_MAIL_THREADS.get(key)
                        if thread and thread.is_alive():
                            continue
                        thread = threading.Thread(
                            target=_demir_mail_worker, args=(rid, role),
                            daemon=True, name=f"onoi-demir-mail-{role}-{rid}",
                        )
                        _DEMIR_MAIL_THREADS[key] = thread
                    thread.start()
                    print(f"[DEMIRMAIL] worker started requisite={rid} role={role} login={acc['login']}", flush=True)
        except Exception as exc:
            print(f"[DEMIRMAIL] supervisor_error {exc}", flush=True)
        time.sleep(5.0)


threading.Thread(target=_demir_mail_supervisor, daemon=True, name="onoi-demir-mail").start()


def _demir_mail_probe(acc: dict[str, Any], mail_cfg: dict[str, Any], limit: int = 5) -> dict[str, Any]:
    """Проверка ящика из админки: логин + разбор последних писем, без зачислений."""
    started = time.monotonic()
    conn = _demir_mail_connect(acc, timeout=20.0)
    login_ms = int((time.monotonic() - started) * 1000)
    try:
        since = (now() - timedelta(days=2)).strftime("%d-%b-%Y")
        typ, data = conn.uid("SEARCH", None, "SINCE", since)
        uids = [int(x) for x in (data[0].split() if (typ == "OK" and data and data[0]) else [])]
        preview: list[dict[str, Any]] = []
        for uid in sorted(uids)[-int(limit):]:
            typ, payload = conn.uid("FETCH", str(uid), "(BODY.PEEK[])")
            if typ != "OK" or not payload or not isinstance(payload[0], tuple):
                continue
            msg = _email_mod.message_from_bytes(payload[0][1])
            subject = _demir_mail_header(msg.get("Subject"))
            sent = _demir_mail_sent_at(msg)
            item = {
                "uid": uid,
                "from": _demir_mail_header(msg.get("From"))[:120],
                "subject": subject[:160],
                "date": str(msg.get("Date") or ""),
                "age_seconds": round((now() - sent).total_seconds(), 1) if sent else None,
                "accepted": not _demir_mail_accept(msg, subject, mail_cfg),
            }
            try:
                item["amount"] = _demir_mail_amount((subject + "\n" + _demir_mail_body(msg)).strip(), mail_cfg)
            except Exception as exc:
                item["amount"] = None
                item["error"] = str(exc)[:120]
            preview.append(item)
        return {
            "ok": True, "login_ms": login_ms, "idle_supported": b"IDLE" in (conn.capabilities and b" ".join(x.encode() for x in conn.capabilities) or b""),
            "total_recent": len(uids), "messages": list(reversed(preview)),
        }
    finally:
        try:
            conn.close()
...
@app.get("/api/macro/mail/status")
async def demir_mail_status(request: Request):
    get_session(request)
    cfg = reload_config()
    mail_cfg = _demir_mail_cfg(cfg)
    accounts = []
    for acc in _demir_mail_accounts(cfg):
        state = _demir_mail_status_for(acc["requisite_id"])
        accounts.append({
            "requisite_id": acc["requisite_id"], "name": acc["name"], "login": acc["login"],
            "host": acc["host"], "port": acc["port"], "ssl": acc["ssl"], "folder": acc["folder"],
            **state,
        })
    return {
        "ok": True,
        "settings": {k: v for k, v in mail_cfg.items()},
        "payment_mode": _payment_check_mode(),
        "accounts": accounts,
    }


@app.post("/api/macro/requisites/{rid}/mail-test")
async def demir_mail_test(rid: str, request: Request):
    get_session(request)
    cfg = reload_config()
    mail_cfg = _demir_mail_cfg(cfg)
    row = next((x for x in (cfg.get("macro") or {}).get("requisites", []) or [] if str(x.get("id")) == rid), None)
    if not row:
        raise HTTPException(404, "Реквизит не найден")
    acc = _demir_mail_account(row, mail_cfg)
    if not acc:
        raise HTTPException(400, "Для реквизита не заданы почта и пароль (или он не demir)")
    try:
        result = await _asyncio.to_thread(_demir_mail_probe, acc, mail_cfg, 5)
    except Exception as exc:
        raise HTTPException(400, f"IMAP: {str(exc)[:200]}")
    return {"ok": True, "login": acc["login"], "host": acc["host"], "port": acc["port"], **result}


@app.post("/api/macro/mail/settings")
async def demir_mail_settings_save(request: Request):
    sess = get_session(request)
    d = await request.json()
    cfg = reload_config()
    block = cfg.setdefault("demir_mail", {})
    for key in ("enabled", "imap_ssl", "idle_enabled", "mark_seen"):
        if key in d:
            block[key] = bool(d[key])
    for key in ("imap_host", "folder", "amount_regex"):
        if key in d:
            block[key] = str(d[key] or "").strip()
    for key in ("imap_port", "idle_timeout_seconds", "lookback_minutes", "reconnect_seconds", "watchers"):
        if key in d:
            try:
                block[key] = int(d[key])
            except Exception:
                raise HTTPException(422, f"Некорректное значение {key}")
    for key in ("poll_seconds", "fast_poll_seconds", "burst_seconds"):
        if key in d:
            try:
    ...
    for key in ("senders", "subject_hints", "bank_types"):
        if key in d:
            ...
            if isinstance(value, str):
                value = value.split(",")
            block[key] = [str(x).strip() for x in (value or []) if str(x).strip()]
    save_config(cfg)
    add_log("Почта Demir: настройки", current_operator(sess), "info")
    return {"ok": True, "settings": _demir_mail_cfg(cfg)}
# === /LUX DEMIR MAIL PAYMENTS (10.46) ========================================
...
async def macro_req_upload(
    ...
    imap_host: str = _Form(""),
    imap_port: int = _Form(0),
    imap_ssl: bool = _Form(True),
    imap_folder: str = _Form(""),
    mail_enabled: bool = _Form(True),
...
async def macro_req_add(request: Request):
    ...
    row = {
        ...
        "imap_port": int(d.get("imap_port") or 0),
        "imap_ssl": bool(d.get("imap_ssl", True)),
        "imap_folder": str(d.get("imap_folder") or "").strip(),
        "mail_enabled": bool(d.get("mail_enabled", True)),
...
async def macro_req_edit(rid: str, request: Request):
    ...
    for key in ("name", "enabled", "bank_type", "email", "mail_password", "logo_url",
                "imap_host", "imap_port", "imap_ssl", "imap_folder", "mail_enabled"):
        if key in d:
            if key in ("enabled", "imap_ssl", "mail_enabled"):
                row[key] = bool(d[key])
            elif key == "imap_port":
                try:
                    row[key] = int(d[key] or 0)
                except Exception:
                    raise HTTPException(422, "imap_port должен быть числом")
...
# ---------------------------------------------------------------------------
# OnoiPay v10.20 — быстрый раздел «Выписка»
# ---------------------------------------------------------------------------

# === ONOIPAY OPTIMA DIRECT PAYMENT v10.28 ===
    ...
    *,
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "Accept": "application/json",
            "User-Agent": "OnoiPay/Optima-Bridge",
        },
        method="GET",
    )
...
# === ONOIPAY NONBLOCKING PROVIDER CREDIT v1 ===
_PROVIDER_CREDIT_EXECUTOR = ThreadPoolExecutor(
    max_workers=32,
    thread_name_prefix="onoi-credit",
)
...
# === ONOIPAY FAST HYBRID ENGINE v11 ===
# Provider POST выполняется один раз. Никаких автоматических повторов POST здесь нет.
_LUX_FAST_CREDIT_EXECUTOR = ThreadPoolExecutor(max_workers=32, thread_name_prefix="onoi-fast-credit")
...
threading.Thread(target=_statement_payment_worker, daemon=True, name="onoi-optima-direct-payment").start()
...
# === OnoiPay v10.40 LOCAL FIRST LINE SUPPORT ===
...
async def _ai_model_decision(chat_id: int, text: str, file_url: str, txs: list[dict]):
    ...
    system=(
      "Ты первая линия поддержки OnoiPay. Отвечай кратко, профессионально, без канцелярита, на языке клиента: русский или кыргызский. "
    ...
    payload={
      ...
      "text":{"format":{"type":"json_schema","name":"onoipay_support_decision","strict":True,"schema":schema}},
...
# === ONOIPAY WEB PUSH + PROVIDER API LOGS v1 ===
...
_LUX_PUSH_STORAGE = STORAGE / "push"
...
async def lux_push_service_worker():
    ...
    return _LuxPushResponse(
        ...
        headers={"Cache-Control":"no-cache, no-store, must-revalidate","Service-Worker-Allowed": (PREFIX + "/") if PREFIX else "/"},
    )
...
async def lux_push_test(request: Request):
    ...
    _lux_push_enqueue(
        ...
        _lux_push_payload(
            ...
            "Уведомления подключены к серверу OnoiPay.",
...
# === ONOIPAY SERVCUL DUPLICATE RECOVERY WORKER v2 ===
...
# === ONOIPAY ADMIN: управление БК, правка и отмена заявок, аудит ============
...
def _adm_init() -> None:
    with _DB_LOCK, _db_conn() as c:
        ...
        CREATE INDEX IF NOT EXISTS idx_admin_audit_target ON admin_audit(target,id);
        """)
...
def _adm_audit(operator: str, action: str, target: str = "", detail: str = "") -> None:
    try:
        with _DB_LOCK, _db_conn() as c:
            c.execute(
                "INSERT INTO admin_audit(operator,action,target,detail,created_at) VALUES(?,?,?,?,?)",
...
async def adm_tx_get(tx_id: str, request: Request):
    ...
    return {
        ...
        "bookmakers": [{"key": k, "label": str((b or {}).get("label") or k.upper()), "enabled": bool((b or {}).get("enabled", True))} for k, b in (cfg.get("bookmakers") or {}).items()],
    }
...
async def adm_tx_edit(tx_id: str, request: Request):
    ...
    if "bookmaker" in d:
        ...
        if cand and cand != new_bk:
            if cand not in (cfg.get("bookmakers") or {}):
...
if __name__ == "__main__":
    uvicorn.run("server:root_app", host=HOST, port=PORT, reload=False, proxy_headers=True, forwarded_allow_ips="127.0.0.1")
